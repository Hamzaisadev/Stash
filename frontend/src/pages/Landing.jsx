...
function MagneticButton({ children, className = '', href = '#' }) {
  ...
  return (
    <a ref={btnRef} href={href} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className={className}>
      {children}
    </a>
  );
}
...
function Landing() {
  ...
  useEffect(() => {
    ...
    const tw = gsap.to(el, { xPercent: -50, duration: 25, ease: 'none', repeat: -1 });
    return () => tw.kill();
  }, []);
  ...
  /* ── Data ── */
  const steps = [
    { icon: <Upload className="w-6 h-6" />, title: 'Drop your files', desc: 'Drag and drop anything — photos, videos, documents, zip files. No size limits ever.' },
    { icon: <Send className="w-6 h-6" />, title: 'Share the room link', desc: 'Send the room link to whoever you want, or let nearby devices find you automatically.' },
    { icon: <Zap className="w-6 h-6" />, title: 'Instant delivery', desc: 'Files go directly to the other device. No middle-man, no upload wait, no cloud.' },
  ];

  const features = [
    { icon: <Monitor className="w-6 h-6" />, title: 'Live Screen & Camera', desc: 'Show your screen or face live to everyone in the room. One click to go fullscreen.' },
    { icon: <Lock className="w-6 h-6" />, title: 'Password Locked Rooms', desc: 'Protect rooms with rotating keys. Only people you give the code to can enter.' },
    { icon: <Flame className="w-6 h-6" />, title: 'Self-Destructing Files', desc: 'Files auto-delete after a timer or vanish the instant someone downloads them.' },
    { icon: <Copy className="w-6 h-6" />, title: 'Shared Clipboard', desc: 'Copy text, links, or notes on one device and grab them immediately on another.' },
    { icon: <Shield className="w-6 h-6" />, title: 'Guest Approvals', desc: 'You control who enters. Approve or deny join requests before anyone gets access.' },
    { icon: <Smartphone className="w-6 h-6" />, title: 'Any Device, Any Browser', desc: 'Works on phones, tablets, and laptops. No app store downloads needed.' },
  ];

  const useCases = [
    { icon: <Laptop className="w-7 h-7" />, title: 'Classrooms', desc: 'Teachers share lecture slides, students submit assignments — all in one temporary room.', color: 'bg-blue-50 text-blue-500' },
    { icon: <Users className="w-7 h-7" />, title: 'Meetings', desc: 'Quick file handoffs during calls. Share screenshots, designs, and docs without email.', color: 'bg-amber-50 text-amber-500' },
    { icon: <Globe className="w-7 h-7" />, title: 'Travel', desc: 'Send photos and boarding passes between your phone and a borrowed laptop instantly.', color: 'bg-emerald-50 text-emerald-500' },
    { icon: <FileText className="w-7 h-7" />, title: 'Events', desc: 'Share slides, brochures, and resources with attendees. Room auto-cleans after.', color: 'bg-purple-50 text-purple-500' },
  ];
  ...
  const marqueeItems = [
    'No Sign Up', 'Direct Transfer', 'Auto Discovery', 'No Size Limits',
    'Temporary Rooms', 'Self-Destructing', 'Screen Sharing', 'Voice Notes',
  ];
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 overflow-x-hidden font-sans selection:bg-red-100 selection:text-red-600">
      ...
      >
        <div className="max-w-7xl mx-auto px-5 md:px-8 h-14 flex items-center justify-between">
          <a href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="text-lg font-extrabold tracking-tight text-slate-900">Stash</span>
          </a>
      ...
      {/* ════════════════════════════════════════════
          2 — HERO
      ════════════════════════════════════════════ */}
      <section ref={heroRef} className="relative min-h-[100svh] flex flex-col items-center justify-center px-6 pt-16 overflow-hidden">
        {/* Decorative rings */}
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <span className="text-[11px] font-semibold text-red-600 uppercase tracking-wider">Now Available</span>
          </motion.div>
          ...
          >
            Drop files.<br />
            <span className="text-red-500">Not patience.</span>
          </motion.h1>
          ...
          >
            Create a temporary room, share the link, and send anything directly to another device. No accounts. No cloud. Nothing left behind.
          </motion.p>
          ...
          >
            ...
            <a href="#how-it-works" className="inline-flex items-center gap-2 text-slate-500 text-[15px] font-semibold hover:text-slate-900 transition-colors cursor-pointer">
              See how it works
      ...
      <section className="py-6 border-y border-slate-100 bg-slate-50/80 overflow-hidden">
        <div className="flex whitespace-nowrap" ref={marqueeRef}>
          {[...marqueeItems, ...marqueeItems, ...marqueeItems, ...marqueeItems].map((item, i) => (
            <span key={i} className="inline-flex items-center gap-3 mx-5 text-[13px] font-bold text-slate-300 uppercase tracking-widest select-none shrink-0">
        ...
        <div className="max-w-6xl mx-auto">
          <Reveal>
            <div className="text-center mb-16">
              <span className="text-[11px] font-bold uppercase tracking-widest text-red-500 mb-3 block">How it works</span>
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight text-slate-900">Three steps. That's it.</h2>
            </div>
          </Reveal>
              ...
              <Reveal key={i} delay={i * 0.1}>
                ...
                >
                  ...
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-5 transition-colors duration-300 ${
                    activeStep === i ? 'bg-red-500 text-white' : 'bg-slate-100 text-slate-500 group-hover:bg-red-50 group-hover:text-red-500'
                  }`}>
                    {step.icon}
                  </div>
                  ...
                  <div className="mt-6 h-1 bg-slate-100 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      transition={{ duration: activeStep === i ? 3 : 0.3, ease: activeStep === i ? 'linear' : 'easeOut' }}
                    />
                  </div>
                </motion.div>
              </Reveal>
      ...
      <section className="py-24 md:py-32 px-6 bg-slate-50/60">
        <div className="max-w-6xl mx-auto">
          <Reveal>
            <div className="text-center mb-16">
              <span className="text-[11px] font-bold uppercase tracking-widest text-red-500 mb-3 block">Features</span>
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight text-slate-900">Everything you need,<br className="hidden sm:block" /> nothing you don't.</h2>
            </div>
          </Reveal>
      ...
      <section className="py-24 md:py-32 px-6 overflow-hidden">
        <div className="max-w-6xl mx-auto">
          <Reveal>
            <div className="text-center mb-16">
              <span className="text-[11px] font-bold uppercase tracking-widest text-red-500 mb-3 block">Preview</span>
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight text-slate-900">Designed to get<br /> out of your way.</h2>
              <p className="text-base text-slate-500 max-w-md mx-auto mt-4 font-medium">A clean, minimal interface that works equally well on your phone and your laptop.</p>
            </div>
          </Reveal>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/[0.06] overflow-hidden">
                ...
                <div className="bg-[#0a0a0f] p-6 md:p-10 min-h-[320px] flex flex-col items-center justify-center text-center gap-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-white font-bold text-sm">my-room</span>
                    <span className="text-[9px] font-mono text-slate-500 bg-slate-900 px-2 py-0.5 rounded-full">3 online</span>
                  </div>
                  {/* Mock file cards */}
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 w-full max-w-md">
                    {['presentation.pdf', 'photo-001.jpg', 'notes.txt'].map((name, i) => (
                      <motion.div
                        key={name}
                        initial={{ opacity: 0, y: 20 }}
                      ...
                      >
                        ...
                        <p className="text-[10px] font-semibold text-slate-300 truncate">{name}</p>
                        <p className="text-[8px] text-slate-600 mt-0.5">{(Math.random() * 5 + 0.5).toFixed(1)} MB</p>
                      </motion.div>
                    ))}
                  </div>
                  <div className="flex items-center gap-2 mt-3">
                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                    <span className="text-[10px] font-semibold text-slate-500">All files shared in real time</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="rounded-3xl border-2 border-slate-200 bg-white shadow-2xl shadow-slate-900/[0.08] overflow-hidden">
                    <div className="bg-[#0a0a0f] p-4 min-h-[200px] flex flex-col gap-2">
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <span className="text-[9px] font-bold text-white">my-room</span>
                      </div>
                      {['photo-001.jpg', 'notes.txt'].map((name) => (
                        <div key={name} className="bg-slate-900 border border-slate-800 rounded-lg p-2">
                          <p className="text-[8px] font-semibold text-slate-300 truncate">{name}</p>
                        </div>
                      ))}
                      <div className="mt-auto flex justify-center">
                        <span className="text-[7px] font-semibold text-emerald-400">● Connected</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <section className="py-24 md:py-32 px-6">
        <div className="max-w-6xl mx-auto">
          <Reveal>
            <div className="text-center mb-16">
              <span className="text-[11px] font-bold uppercase tracking-widest text-red-500 mb-3 block">Use cases</span>
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight text-slate-900">Built for real situations.</h2>
            </div>
          </Reveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {useCases.map((uc, i) => (
              <Reveal key={i} delay={i * 0.08}>
                ...
                >
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${uc.color} group-hover:scale-110 transition-transform duration-300`}>
                    {uc.icon}
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 mb-1">{uc.title}</h3>
        ...
        <Reveal>
          <div className="max-w-3xl mx-auto text-center relative z-10">
            ...
            <p className="text-lg text-slate-500 max-w-md mx-auto mb-10 font-medium">
              No downloads. No sign ups. Just open a room and start sending files to any device, instantly.
            </p>
            <MagneticButton
              href="/rooms"
      ...
      <footer className="border-t border-slate-100 bg-white py-10 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[15px] font-extrabold text-slate-900">Stash</span>
          </div>
