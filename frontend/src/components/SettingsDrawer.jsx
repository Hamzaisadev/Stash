    ...
    return (
        <div className="fixed inset-y-0 right-0 w-80 bg-[#1C1C1E] border-l border-gray-800 shadow-2xl z-50 flex flex-col transform transition-transform duration-300">
            <div className="p-4 border-b border-gray-800 flex items-center justify-between">
                ...
                <button onClick={onClose} className="p-1 text-gray-400 hover:text-white rounded-full hover:bg-gray-800 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-8">
                ...
                <div className="space-y-4 pt-4 border-t border-gray-800">
                    <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Security</h3>
                    ...
                    <label className="flex items-center justify-between cursor-pointer">
                        ...
                        <input 
                            ...
                            onChange={(e) => updateRoomSettings(room.id, { is_protected: e.target.checked })}
                        />
                    </label>

                    {room.is_protected && (
                        <div className="bg-[#111111] p-3 rounded-xl border border-gray-800">
                            <div className="text-xs text-gray-400 mb-1">Room Key (6 chars)</div>
                            <div className="flex items-center justify-between">
                                <span className="font-mono text-lg font-bold text-white tracking-widest">{room.stack_key || '------'}</span>
                                <button onClick={() => rotateStackKey(room.id)} className="text-blue-400 hover:text-blue-300 text-xs px-2 py-1 rounded bg-blue-500/10 transition-colors">
                                    Rotate
                                </button>
                            </div>
                        </div>
                    ...
                    <label className="flex items-center justify-between cursor-pointer">
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs text-gray-500">Require host to approve joins</span>
                        </div>
                        <input 
                            type="checkbox" 
                            className="form-checkbox h-5 w-5 text-blue-500 rounded bg-gray-700 border-gray-600 focus:ring-blue-500 focus:ring-offset-gray-900"
                            checked={room.accept_only}
                            onChange={(e) => updateRoomSettings(room.id, { accept_only: e.target.checked })}
                        />
                    </label>
                </div>
                ...
                {requests.length > 0 && (
                    <div className="space-y-3 pt-4 border-t border-gray-800">
                        ...
                        <div className="space-y-2">
                            {requests.map(req => (
                                <div key={req.clientId} className="bg-[#2A2A2D] p-3 rounded-xl flex items-center justify-between border border-gray-700">
                                    <div className="flex flex-col truncate pr-2">
                                        <span className="text-sm font-medium text-white truncate">{req.guestName}</span>
                                        <span className="text-xs text-gray-500 font-mono truncate">{req.clientId.substring(0, 12)}...</span>
                                    </div>
                                    <div className="flex items-center gap-1 shrink-0">
                                        <button onClick={() => handleApprove(req.clientId, req.guestSocketId)} className="p-1.5 bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 transition-colors">
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                        </button>
                                        <button onClick={() => handleDeny(req.clientId, req.guestSocketId)} className="p-1.5 bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors">
