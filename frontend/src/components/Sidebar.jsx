    ...
    return (
        <ShadcnSidebar className="border-r border-slate-200 bg-white select-none text-slate-800">
            <SidebarHeader className="p-4 border-b border-slate-200/50">
                ...
                >
                    <FolderHeart className="w-5.5 h-5.5 text-red-500 fill-red-500/20" />
                    <span className="tracking-tight text-slate-900 font-extrabold">Stash</span>
                </div>
            </SidebarHeader>
                        ...
                        <SidebarMenu className="space-y-1">
                            {[
                                ...
                                <SidebarMenuItem key={item.id}>
                                    ...
                                    >
                                        <span className={activeMode === item.id ? "text-white" : "text-slate-400"}>
                                            {item.icon}
                                        </span>
                                        <span className="flex-grow">{item.label}</span>
                                        {item.count !== null && item.count > 0 && (
                                            <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded-full transition-all ${
                                                activeMode === item.id ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'
                                            }`}>
                                                {item.count}
                                            </span>
                                        )}
                ...
                <SidebarGroup>
                    <SidebarGroupLabel className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 px-1 flex justify-between items-center w-full">
                        <span className="flex items-center gap-1.5">
                            My Rooms
                            <span className="text-[9px] font-normal text-slate-400 lowercase font-sans">({myRooms.length}/5 max)</span>
                        </span>
                        <div className="flex items-center space-x-1.5 text-slate-400">
                            <button onClick={() => setShowCreateModal(true)} className="hover:text-slate-700 transition-colors p-0.5 cursor-pointer" title="Create Room">
                                <Plus className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={onOpenJoinModal} className="hover:text-slate-700 transition-colors p-0.5 cursor-pointer" title="Join Room Manually">
                    ...
                    <SidebarGroupContent className="space-y-2">
                        <SidebarMenu className="space-y-1">
                            {myRooms.map(r => (
                                <SidebarMenuItem key={r.id} className="group/item relative flex items-center justify-between rounded-xl hover:bg-slate-100 transition-all">
                                    <SidebarMenuButton
                                        ...
                                        className={`flex-grow text-left px-2.5 py-2 text-xs truncate flex items-center gap-2 cursor-pointer transition-colors ${
                                            currentRoomId === r.id ? 'bg-slate-100 text-slate-900 font-semibold rounded-xl' : 'text-slate-655 hover:text-slate-900'
                                        }`}
                                    >
                            ...
                            {myRooms.length === 0 && (
                                <li className="px-2.5 py-1 text-[11px] text-slate-450">No rooms created yet.</li>
                            )}
                        </SidebarMenu>
                    </SidebarGroupContent>
                </SidebarGroup>
            ...
            {/* Create Room Modal */}
            <Dialog open={showCreateModal} onOpenChange={(open) => { if (!isSubmitting) setShowCreateModal(open); }}>
              <DialogContent className="bg-white border-slate-200 text-slate-900">
                <DialogHeader>
                  <DialogTitle className="text-sm font-bold text-slate-900">Create New Room</DialogTitle>
                  <DialogDescription className="text-xs text-slate-500">
                    Setup a custom workspace name, description, ID, and security configuration.
                  </DialogDescription>
                </DialogHeader>
                <form onSubmit={handleCreateRoomSubmit} className="space-y-4 text-xs text-slate-750">
                  {/* Room Name */}
                  <div className="space-y-1">
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Room Name *</label>
                    <Input 
                      type="text" 
                      autoFocus
                  ...
                  {/* Custom Room ID */}
                  <div className="space-y-1">
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Custom Room ID</label>
                    <Input 
                      type="text" 
                  ...
                  <div className="border-t border-slate-200 pt-3 space-y-3">
                    ...
                    <div className="flex items-center justify-between border border-slate-100 bg-slate-55/60 p-2.5 rounded-xl">
                      <div className="flex flex-col text-left">
                        <span className="text-xs font-semibold text-slate-800">Private Room</span>
                        <span className="text-[9px] text-slate-500">Require guests to input a passcode pin to enter</span>
                      </div>
                      <input 
                        type="checkbox" 
                        checked={isProtected}
                    ...
                    {isProtected && (
                      <div className="bg-white border border-slate-200 rounded-xl p-3 space-y-2 text-left animate-slideDown">
                        <label className="text-[10px] text-slate-450 font-bold uppercase tracking-wider block">Custom Access Pin / Passcode</label>
                        <Input 
                          type="text"
                          maxLength={12}
                    ...
                    <div className="flex items-center justify-between border border-slate-100 bg-slate-55/60 p-2.5 rounded-xl">
                      <div className="flex flex-col text-left">
                        <span className="text-xs font-semibold text-slate-800">Manual Host Approval</span>
                        <span className="text-[9px] text-slate-500">Host must approve each guest join request manually</span>
                      </div>
                      <input 
                        type="checkbox" 
                        checked={acceptOnly}
