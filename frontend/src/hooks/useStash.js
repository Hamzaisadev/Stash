...
const SOCKET_BASE = import.meta.env.PROD
  ? PROD_BACKEND_URL
  : `http://${serverIP}:5000`;
...
const getAuthHeaders = (roomId = null, isJson = false) => {
  ...
  if (roomId) {
    const tokens = JSON.parse(
      localStorage.getItem("stash_room_tokens") || "{}",
    );
...
export function useStash() {
  ...
  const [myRooms, setMyRooms] = useState(() =>
    JSON.parse(localStorage.getItem("stash_my_rooms") || "[]"),
  );
  const [joinedRooms, setJoinedRooms] = useState(() =>
    JSON.parse(localStorage.getItem("stash_joined_rooms") || "[]"),
  );
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      setTimeout(() => {
        if (socketRef.current && roomRef.current.id) {
          const isHost =
            roomRef.current.creator_socket_id === generateClientId() ||
          ...
          console.log(
            "Re-emitted join-room on connect:",
            roomRef.current.id,
            "isHost:",
            isHost,
          );
    ...
    socket.on("file-uploaded", (newFile) => {
      ...
      fireNotification(
        "New file received",
        `${newFile.filename} was shared in your room.`,
      );
    ...
    socket.on("join-request", (requestData) => {
      ...
      if (requestData.hostClientId && requestData.hostClientId !== myClientId)
        return;
      window.dispatchEvent(
        new CustomEvent("stash-join-request", { detail: requestData }),
      );
      fireNotification(
        "Access Request",
        `${requestData.guestName} wants to join your room.`,
      );
    });
    ...
    socket.on("join-approved", ({ token, roomId }) => {
      ...
      window.dispatchEvent(
        new CustomEvent("stash-join-approved", { detail: { roomId } }),
      );
      fireNotification(
        "Access Granted",
    ...
    socket.on("join-denied", ({ roomId }) => {
      window.dispatchEvent(
        new CustomEvent("stash-join-denied", { detail: { roomId } }),
      );
    ...
    socket.on("clipboard-sync", ({ text, senderClientId }) => {
      ...
      setClipboard((prev) => [
        ...prev,
        { text, time: Date.now(), fromRemote: true },
      ]);
    ...
    socket.on("file-downloaded", ({ fileId, downloadCount }) => {
      setRoom((prev) => ({
        ...
        files: prev.files.map((f) =>
          f.id === fileId ? { ...f, download_count: downloadCount } : f,
        ),
      }));
    });
    ...
    socket.on("screen-share-signal", async ({ signal, senderId }) => {
      try {
        if (signal.type === "screen-request") {
          ...
          const pc =
            screenSharePCs.current[senderId] || receiverScreenPC.current;
          if (pc) {
  ...
  const fetchDefaultRoom = async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        setGate({
          require_password: json.require_password,
          accept_only: json.accept_only,
    ...
    try {
      ...
      if (json.status === "success") {
        ...
        setJoinedRooms((prev) => {
          ...
          if (inJoinedRooms) {
            return prev.map((r) =>
              r.id === roomInfo.id ? { ...r, name: roomInfo.name } : r,
            );
          }
    ...
    try {
      ...
      if (json.status === "success") {
        ...
        setMyRooms((prev) => [
          ...
          { id: newRoomId, name: json.data.room.name },
        ]);
        setRoom((prev) => ({
          ...prev,
          id: newRoomId,
  ...
  const approveGuest = (roomId, guestSocketId, guestClientId) => {
    if (socketRef.current) {
      socketRef.current.emit("join-approve", {
        roomId,
        guestSocketId,
        guestClientId,
        hostId: clientId,
  ...
  const denyGuest = (roomId, guestSocketId) => {
    if (socketRef.current) {
      socketRef.current.emit("join-deny", {
        roomId,
        guestSocketId,
        hostId: clientId,
      });
  ...
  const joinRoomChannel = useCallback(() => {
    if (socketRef.current && room.id) {
      const isHost =
        room.creator_socket_id === generateClientId() ||
    ...
    try {
      ...
      if (json.status === "success" || (json.data && json.data.id)) {
        ...
        const myUploads = JSON.parse(
          localStorage.getItem("stash_my_uploads") || "[]",
        );
  ...
  const downloadFile = async (file, filePassword = "") => {
    ...
    if (socketRef.current && file.file_size <= MAX_P2P_FILE_SIZE) {
      ...
      try {
        ...
        console.warn(
          `P2P failed for ${file.filename}, falling back to cloud:`,
          p2pErr,
        );
      }
    }
  ...
  const sendClipboard = (text) => {
    ...
    setClipboard((prev) => [
      ...prev,
      { text, time: Date.now(), fromRemote: false },
    ]);
  };
